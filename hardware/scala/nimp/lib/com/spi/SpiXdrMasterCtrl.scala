...
//      rsp.payload <> accessBus.rsp.data
  ...
  def driveFrom(toplevel : TopLevel, bus : BusSlaveFactory, baseAddress : Int = 0)(mapping : MemoryMappingParameters) = new Area {
    ...
    //Configs
    bus.driveAndRead(config.kind, baseAddress + 8, bitOffset = 0)
    bus.driveAndRead(config.mod, baseAddress + 8, bitOffset = 4)
    bus.driveAndRead(config.sclkToogle, baseAddress + 0x20)
    if(p.ssGen) {
      bus.driveAndRead(config.ss.setup, baseAddress + 0x24)
      bus.driveAndRead(config.ss.hold, baseAddress + 0x28)
      bus.driveAndRead(config.ss.disable, baseAddress + 0x2C)
      bus.driveAndRead(config.ss.activeHigh, baseAddress + 0x30)
    }
